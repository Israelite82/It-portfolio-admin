...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      {/* ── TOP BAR ── */}
      <div className="bg-white border-b border-gray-200 px-4 sm:px-8 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 sm:ml-6">
        <p className="text-[15px] text-gray-700 font-medium tracking-wide">
          Welcome back,{" "}
          <span className="font-semibold text-[#1a1612]">Admin</span>
        </p>
        <button className="bg-[#6366F1] hover:bg-[#6a5dbf] text-white text-sm font-semibold px-5 py-2.5 sm:mr-10 rounded-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md w-full sm:w-auto">
          + Quick Add
        </button>
      </div>

      {/* ── MAIN CONTENT ── */}
      <div className="px-4 sm:px-8 py-8 space-y-8 sm:space-y-14">
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            {stats.map((stat) => (
              ...
              >
                <p className="text-md text-gray-700 font-medium tracking-wide mb-4 sm:mb-8">
                  {stat.label}
                </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:gap-8 gap-4">
            {quickActions.map((action) => (
              <button
                key={action.label}
                onClick={() => navigate(action.path)}
                className={`${action.bg} ${action.text} border ${action.border} rounded-xl py-4 px-8 text-sm font-semibold transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md w-full lg:w-auto`}
              >
                {action.label}
              </button>
            ))}
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-[#FEF3C7] rounded-xl px-4 sm:px-6 py-5 mb-8 shadow-lg">
            <div className="space-y-3">
